...
export const PRR_STATUSES = ["received", "acknowledged", "in_progress", "extended", "closed"];
...
const ALLOWED_ACCESS_REQUEST_TRANSITIONS = {
...
export class PrrStore {
    ...
    `);
        const persist = this.db.transaction((publicId) => {
            const metadata = JSON.stringify(input.metadata ?? {});
            insertPrr.run(prrId, publicId, input.tenantId, normalizeOptionalText(input.requesterName), normalizeOptionalText(input.requesterEmail), input.subject.trim(), normalizeOptionalText(input.description), nowIso, dueAtIso, nowIso);
    ...
    intakeAccessRequest(input) {
      ...
      VALUES
        (?, ?, ?, 'queued', ?, ?, NULL)
      `);
        const persist = this.db.transaction((caseId) => {
            ...
            insertRequest.run(requestId, caseId, input.tenantId, normalizedRequesterName, normalizedRequesterEmail, normalizedOrganization, normalizedRequestedRole, systemName, normalizedJustification, nowIso, nowIso, normalizedRequestedByUserId);
    ...
    transitionStatus(args) {
        const transition = this.db.transaction(() => {
            const row = this.db.prepare("SELECT * FROM prr WHERE id = ? AND tenant_id = ?").get(args.id, args.tenantId);
            if (!row) {
                return { ok: false, code: "not_found" };
            }
            const allowed = ALLOWED_TRANSITIONS[row.status] ?? [];
            if (!allowed.includes(args.toStatus)) {
                return { ok: false, code: "invalid_transition", fromStatus: row.status };
            }
            const nowIso = new Date().toISOString();
            this.db
          ...
          VALUES
            (?, ?, 'status_change', ?, ?, ?, ?, ?)
          `)
                .run(args.id, args.tenantId, args.actorUserId, row.status, args.toStatus, JSON.stringify(args.metadata ?? {}), nowIso);
            return {
                ok: true,
                row: {
                    id: args.id,
    ...
    closeCase(args) {
        const transition = this.db.transaction(() => {
            ...
            this.db
                .prepare("UPDATE prr SET status = 'closed', closed_at = ?, disposition = ?, last_action_at = ? WHERE id = ? AND tenant_id = ?")
          ...
          VALUES
            (?, ?, 'close', ?, ?, 'closed', ?, ?)
          `)
                .run(args.id, args.tenantId, args.actorUserId, row.status, JSON.stringify(args.metadata ?? {}), nowIso);
            return {
                ok: true,
                row: {
                    id: args.id,
    ...
    closeAccessRequest(args) {
        const transition = this.db.transaction(() => {
            ...
            this.db
                .prepare("UPDATE access_request SET status = 'closed', closed_at = ?, resolution = ?, last_action_at = ? WHERE id = ? AND tenant_id = ?")
    ...
    getRecordForTenant(prrId, tenantId) {
        const row = this.db
    ...
    getAccessRequestForTenant(accessRequestId, tenantId) {
        const row = this.db
            .prepare("SELECT * FROM access_request WHERE id = ? AND tenant_id = ?")
            .get(accessRequestId, tenantId);
        return row ?? null;
    }
    getAccessRequestAuditCount(accessRequestId) {
        const count = this.db
            .prepare("SELECT COUNT(*) as count FROM access_request_audit WHERE access_request_id = ?")
            .get(accessRequestId);
        return Number.isFinite(count.count) ? count.count : 0;
    }
    getLatestAccessRequestNotification(accessRequestId) {
        const row = this.db
            .prepare(`
    ...
    claimPendingAccessRequestNotifications(limit) {
    ...
    `);
        const claim = this.db.transaction(() => {
            const rows = selectPending.all(nowIso, boundedLimit);
            const claimed = [];
