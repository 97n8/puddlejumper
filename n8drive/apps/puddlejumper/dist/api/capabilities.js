// ── Action definitions ──────────────────────────────────────────────────────
export const PJ_ACTION_DEFINITIONS = [
...
// ── Manifest builder ────────────────────────────────────────────────────────
export function buildCapabilityManifest(auth, runtimeTiles, runtimeCapabilities) {
...
export function isPjActionAllowed(manifest, action) {
    return action.requires.every((k) => manifest.capabilities[k] === true);
}
export function listAllowedPjActions(manifest) {
...
export function assertTenantScope(auth, payload) {
...
// ── Scoped request ID ───────────────────────────────────────────────────────
export function scopedRequestId(userId, tenantId, requestId) {
    if (!requestId)
        return undefined;
...
function resolvePrimaryScope(auth, runtimeContext) {
    ...
    if (fromTenantId)
        return fromTenantId;
    const fromTenants = auth.tenants.find((t) => t.id.trim())?.id.trim();
    if (fromTenants)
        return fromTenants;
...
export function buildPjEvaluatePayload(auth, runtimeContext, request, correlationId) {
    ...
    const archieve = (dept, type) => ({ dept, type, date: dateStamp, seq: 1, v: 1 });
...
// ── Decision helpers ────────────────────────────────────────────────────────
export function resolveDecisionStatusCode(result) {
    if (result.approved)
        return 200;
    if (result.warnings.some((w) => /idempotency conflict|schema version mismatch/i.test(w)))
        return 409;
    if (result.warnings.some((w) => /invalid canonical source|canonical/i.test(w)))
        return 400;
    return 400;
}
export function buildPjExecuteData(request, evaluatePayload, decision) {
