...
import { getAuthContext, requireAuthenticated, requirePermission, requireRole, signJwt, } from "@publiclogic/core";
...
export function createGovernanceRoutes(opts) {
    ...
    router.get("/pj/identity-token", async (req, res) => {
        ...
        const token = await signJwt({ sub: auth.userId, name: auth.name, role: auth.role, permissions: auth.permissions,
            tenants: auth.tenants, tenantId: auth.tenantId ?? undefined, delegations: auth.delegations }, { expiresIn: `${expiresInSeconds}s` });
        if (opts.nodeEnv !== "production") {
