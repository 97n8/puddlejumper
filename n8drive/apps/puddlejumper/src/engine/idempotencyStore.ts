...
export type ClaimResult<TOutput> =
  | { type: "acquired" }
  | { type: "replay"; output: TOutput }
  | { type: "pending"; promise: Promise<TOutput> }
  | { type: "conflict" }
  | { type: "schema_mismatch"; storedSchemaVersion: number };
...
export class IdempotencyStore<TOutput> {
  private readonly db: Database.Database;
  private readonly inFlight = new Map<string, InFlightRecord<TOutput>>();
  ...
  private normalizeSchemaVersion(value: unknown): number {
    const parsed = Number.parseInt(String(value ?? 1), 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : 1;
  }

  private createDeferred<TValue>(): Deferred<TValue> {
    let resolveFn: ((value: TValue) => void) | null = null;
  ...
  private readRow(requestId: string): Row | undefined {
    return this.db
      .prepare(`
  ...
  private async waitForCompletedRow(
    requestId: string,
    payloadHash: string,
    schemaVersion: number
  ): Promise<ClaimResult<TOutput> | null> {
    ...
    while (Date.now() < deadline) {
      ...
      if (row.result_json) {
        return { type: "replay", output: JSON.parse(row.result_json) as TOutput };
      }
  ...
  async claim(
    ...
    if (existingRow) {
      ...
      if (existingRow.result_json) {
        return { type: "replay", output: JSON.parse(existingRow.result_json) as TOutput };
      }
    ...
    if (inserted.changes !== 1) {
      ...
      if (rowAfterInsert.result_json) {
        return { type: "replay", output: JSON.parse(rowAfterInsert.result_json) as TOutput };
      }
      const pendingResult = await this.waitForCompletedRow(requestId, payloadHash, schemaVersion);
      if (pendingResult) {
        return pendingResult;
    ...
    const deferred = this.createDeferred<TOutput>();
  ...
  storeResult(
    ...
    const persist = this.db.transaction(() => {
      const updated = this.db
        .prepare(
          "UPDATE idempotency SET result_json = ?, schema_version = ?, decision_status = ?, decided_at = ? WHERE request_id = ?"
        )
