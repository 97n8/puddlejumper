...
type ProviderAdapter = {
  provider: ConnectorProvider;
  getStatus: () => Promise<ConnectorStatusPayload>;
  getAuthUrl: (signedState: string, redirectUri: string) => string;
  handleCallback: (code: string, redirectUri: string) => Promise<void>;
  disconnect: () => Promise<void>;
  searchResources: (query: string) => Promise<ConnectorResult[]>;
};
...
function parseBearerExpiresAt(expiresInSeconds: unknown): string | null {
...
function isTokenValid(record: ConnectorTokenRecord | null): boolean {
  if (!record) {
    return false;
  }
  if (!record.expiresAt) {
...
async function getJsonWithBearer(
  fetchImpl: typeof fetch,
  url: string,
  accessToken: string,
  headers: Record<string, string> = {}
...
function getMicrosoftAdapter(
  ...
  return {
    ...
    async handleCallback(code, redirectUri) {
      ...
      const profile = await getJsonWithBearer(
        fetchImpl,
    ...
    async searchResources(query) {
      ...
      const endpoint =
        normalizedQuery.length > 0
          ? `https://graph.microsoft.com/v1.0/me/drive/root/search(q='${encodeURIComponent(
            ...
            )}')?$top=25&$select=id,name,webUrl,folder,file`
      ...
      return rawValues
        ...
        .map((entry) => ({
          ...
          type:
            entry.folder && typeof entry.folder === "object"
              ? "folder"
    ...
    async searchResources(query) {
      ...
      const driveQuery =
        normalizedQuery.length > 0
          ...
          : "trashed=false";
      const url = `https://www.googleapis.com/drive/v3/files?pageSize=25&fields=files(id,name,mimeType,webViewLink)&q=${encodeURIComponent(
        driveQuery
      )}`;
      ...
      return files
        ...
        .map((entry) => ({
          ...
          name: typeof entry.name === "string" ? entry.name : "Untitled",
          type:
            typeof entry.mimeType === "string" && entry.mimeType === "application/vnd.google-apps.folder"
              ? "folder"
              : "file",
...
function getGitHubAdapter(
  ...
  return {
    ...
    async handleCallback(code, redirectUri) {
      ...
      const grantedScopes = splitScopes(
        typeof tokenPayload.scope === "string" && tokenPayload.scope.trim() ? tokenPayload.scope : scope
      );
...
function requireAuthContext(req: express.Request, res: express.Response): AuthContext | null {
...
export function createConnectorsRouter(options: CreateConnectorsRouterOptions): express.Router {
  ...
  router.get("/", async (req, res) => {
    ...
    const providers: ConnectorProvider[] = ["microsoft", "google", "github"];
