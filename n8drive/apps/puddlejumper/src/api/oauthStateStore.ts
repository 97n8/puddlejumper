...
export class OAuthStateStore {
  ...
  prune(): number {
    const result = this.db.prepare("DELETE FROM oauth_state WHERE expires_at < ? OR used = 1").run(Date.now());
    return result.changes;
  }

  /** Number of active (non-expired, non-used) states. Useful for tests/diagnostics. */
  count(): number {
    const row = this.db.prepare("SELECT COUNT(*) as cnt FROM oauth_state WHERE expires_at >= ? AND used = 0").get(Date.now()) as { cnt: number };
    return row.cnt;
  }
