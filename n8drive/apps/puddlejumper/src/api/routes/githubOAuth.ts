  ...
  router.get("/auth/github/callback", async (req, res) => {
    ...
    try {
      ...
      await createSessionAndSetCookies(
        res,
        { sub: userInfo.sub, email: userInfo.email, name: userInfo.name, provider: "github" },
        opts.nodeEnv,
      );
      ...
      authEvent(req, "login_failed", { provider: "github", method: "oauth_redirect", reason: err?.message ?? "unknown" });
      const frontendUrl = process.env.FRONTEND_URL || "https://pj.publiclogic.org";
