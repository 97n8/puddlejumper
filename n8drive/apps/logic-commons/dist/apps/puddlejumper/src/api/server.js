...
import { createOptionalJwtAuthenticationMiddleware, createJwtAuthenticationMiddleware, csrfProtection, getAuthContext, requireAuthenticated, requirePermission, requireRole, resolveAuthOptions, signJwt } from "@publiclogic/core";
...
function renderPjWorkspaceHtml(trustedParentOrigins) {
    ...
    if (inlineHashes.styleHash && inlineHashes.scriptHash) {
        ...
        source = source.replace(/<meta http-equiv="Content-Security-Policy" content="[^"]*">/, `<meta http-equiv="Content-Security-Policy" content="${inlineMetaCsp}">`);
    }
...
export async function processAccessNotificationQueueOnce(options) {
    ...
    for (const notification of claimed) {
        ...
        try {
            ...
            const message = truncateText(error instanceof Error ? error.message : "Unknown notification delivery error", 1_000);
...
function buildCapabilityManifest(auth, runtimeTiles, runtimeCapabilities) {
    ...
    const hasCapabilities = runtimeCapabilities !== null &&
...
function buildPjEvaluatePayload(auth, runtimeContext, request, correlationId) {
    ...
    const requestIdFromPayload = (typeof request.payload === "object" && request.payload && "requestId" in request.payload
...
export function createApp(nodeEnv = process.env.NODE_ENV ?? "development", options = {}) {
    ...
    const msGraphTokenExchangeEnabled = process.env.ALLOW_PJ_GRAPH_TOKEN_EXCHANGE === "true" || nodeEnv !== "production";
    ...
    const accessNotificationWorkerIntervalMs = Math.max(5_000, options.accessNotificationWorker?.intervalMs ??
        configuredWorkerInterval ??
        DEFAULT_ACCESS_NOTIFICATION_INTERVAL_MS);
    const accessNotificationBatchSize = Math.max(1, Math.min(100, options.accessNotificationWorker?.batchSize ??
        configuredWorkerBatch ??
        DEFAULT_ACCESS_NOTIFICATION_BATCH_SIZE));
    const accessNotificationMaxRetries = Math.max(1, Math.min(50, options.accessNotificationWorker?.maxRetries ??
    ...
    const loginRateLimit = createRateLimit({
        ...
        keyGenerator: (req) => {
            const username = req.body && typeof req.body === "object" && "username" in req.body && typeof req.body.username === "string"
    ...
    if (!accessNotificationWorkerDisabled && accessNotificationWebhookUrl) {
        const runAccessNotificationWorker = async () => {
            try {
