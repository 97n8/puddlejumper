...
import { createRefreshEntry, verifyRefreshEntry, rotateRefreshEntry, revokeRefreshEntry, } from '../lib/refreshTokens.js';
...
router.post('/login', async (req, res) => {
    try {
        ...
        const baseClaims = { sub: userInfo.sub, email: userInfo.email, name: userInfo.name, provider };
        ...
        const refreshJwt = await signJwt({ ...baseClaims, jti: refreshEntry.jti, token_type: 'refresh' }, { expiresIn: '7d' });
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        const newRefreshJwt = await signJwt({ sub: payload.sub, email: payload.email, name: payload.name, provider: payload.provider, jti: newEntry.jti, token_type: 'refresh' }, { expiresIn: '7d' });
...
router.post('/revoke', async (req, res) => {
    try {
        ...
        const payload = await verifyJwt(refreshToken).catch(() => null);
        if (payload?.jti)
            await revokeRefreshEntry(payload.jti);
        res.clearCookie('pj_refresh', { path: '/api/refresh' });
        return res.status(204).end();
    }
    catch (err) {
