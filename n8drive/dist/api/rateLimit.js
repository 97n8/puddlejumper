...
export function createRateLimit(options) {
  ...
  `);
    ...
    const insertBucket = db.prepare("INSERT OR REPLACE INTO rate_limit_buckets (key, count, reset_at, last_seen) VALUES (?, ?, ?, ?)");
    ...
    const applyRateLimit = db.transaction((key, now) => {
        const row = readBucket.get(key);
