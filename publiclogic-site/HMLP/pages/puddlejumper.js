...
export async function renderPuddleJumper(ctx) {
  ...
  if (!pjCfg?.adminUrl || !pjCfg?.apiUrl) {
    return {
      ...
      content: el("div", { class: "card" }, [
        ...
        el("pre", { style: "white-space: pre-wrap; font-size: 12px; color: var(--muted); margin-top: 12px;" }, [
  ...
  // Async session check — non-blocking
  (async () => {
    try {
      let user = await checkPjSession();
      if (!user) user = await establishPjSession(auth);
      ...
      if (user) {
        sessionStatus.appendChild(el("div", { class: "pj-session-active" }, [
          ...
          el("span", {}, [
            "Session active",
            user.email
              ? el("span", { class: "small", style: "margin-left:8px;" }, [`(${user.email})`])
              : ""
          ].filter(Boolean))
        ]));
        sessionActions.appendChild(
          el("a", { class: "btn btn--pj", href: baseUrl, target: "_blank", rel: "noreferrer" },
            ["Open PJ Admin ⚡"])
        );
      } else {
            ...
            onClick: async () => {
              ...
              try {
                ...
                sessionStatus.appendChild(el("div", {
                  ...
                  el("span", {}, [
                    u ? "Session established" : "SSO bridge failed — open PJ to sign in manually"
                  ])
                ]));
              } catch {
                sessionStatus.innerHTML = "";
        ...
        sessionActions.appendChild(
          el("a", { class: "btn", href: baseUrl, target: "_blank", rel: "noreferrer" },
  ...
  const viewsGrid = el("div", { class: "grid" });
  ...
  const infoRows = [
    ["API Endpoint",     el("code", { class: "pj-info-value" }, [apiUrl])],
    ["Admin URL",        el("code", { class: "pj-info-value" }, [baseUrl])],
    ["SSO Provider",     el("span", { class: "pj-info-value" }, ["Microsoft Entra ID (token exchange)"])],
    ["Token Exchange",   el("code", { class: "pj-info-value" }, [`${apiUrl}/api/auth/token-exchange`])],
    ["Session Check",    el("code", { class: "pj-info-value" }, [`${apiUrl}/api/session`])],
    ["Governance Owner", el("span", { class: "pj-info-value" }, ["PublicLogic (Nate Boudreau, MPA MCPPO)"])],
    ["Encoding Partner", el("span", { class: "pj-info-value" }, ["Polimorphic"])]
  ];

  const infoCard = el("div", { class: "card", style: "grid-column: span 12;" }, [
    el("h3", {}, ["Integration Details"]),
    el("div", { class: "pj-info-grid" },
      infoRows.map(([label, valueEl]) =>
        el("div", { class: "pj-info-row" }, [
          el("span", { class: "pj-info-label" }, [label]),
          valueEl
        ])
      )
    )
  ...
  const grid = el("div", { class: "grid" }, [
    sessionCard, boundaryCard, viewsCard, infoCard
  ]);
  ...
  return {
    ...
    actions: [
      { label: "⚡ Open PuddleJumper", variant: "pj", href: baseUrl }
    ],
    content: grid
  };
}
