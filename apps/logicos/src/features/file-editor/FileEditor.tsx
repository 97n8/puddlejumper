...
export function FileEditor({
  ...
  casespaceId, fileSha, userId, onSave, onSaveMulti, onPublishAsTemplate,
}: FileEditorProps) {
  ...
  useEffect(() => {
    ...
    const svc = new DraftService(initialPath, initialContent, casespaceId, userId)
  ...
  const handleSave = useCallback(async (retryProvider?: string) => {
    ...
    if (results.every(r => r.success)) {
      await draftServiceRef.current?.submit()
