...
import type { CreateDraftRequest, PjDraftRecord, UpdateDraftRequest, VaultFile, VaultFileFull } from './types'

export const builderDomain = {
drafts: {
  list: (casespaceId: string, path?: string): Promise<{ drafts: PjDraftRecord[] }> => {
    const params = new URLSearchParams({ casespaceId })
    if (path) params.set('path', path)
    return pjFetch(`${PJ}/api/files/drafts?${params.toString()}`, { credentials: 'include' }).then(r => r.json())
  },

  get: (draftId: string): Promise<PjDraftRecord> =>
    pjFetch(`${PJ}/api/files/drafts/${encodeURIComponent(draftId)}`, { credentials: 'include' }).then(r => r.json()),

  create: (body: CreateDraftRequest): Promise<{ draft: PjDraftRecord }> =>
    pjFetch(`${PJ}/api/files/drafts`, {
      method: 'POST',
  ...
  update: (draftId: string, body: UpdateDraftRequest): Promise<{ draft: PjDraftRecord }> =>
    pjFetch(`${PJ}/api/files/drafts/${encodeURIComponent(draftId)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      credentials: 'include',
  ...
  submit: (draftId: string): Promise<{ draft: PjDraftRecord }> =>
    pjFetch(`${PJ}/api/files/drafts/${encodeURIComponent(draftId)}/submit`, {
      method: 'POST',
      credentials: 'include',
    }).then(r => r.json()),
},
