...
import type { PrrStore, AccessRequestStatus } from "../prrStore.js";
import {
...
export function createAccessRoutes(opts: AccessRoutesOptions): express.Router {
  ...
  router.post("/access/request/:id/status", requireAuthenticated(), (req, res) => {
    ...
    if (!accessRequestId) { res.status(400).json({ error: "Invalid access request id" }); return; }
    ...
    if (!toStatusParsed.success) { res.status(400).json({ error: "Invalid status transition target" }); return; }
