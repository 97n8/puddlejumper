...
export const RETENTION_MAP: Record<string, { retention: string; route: string }> = {
...
export function isLaunchIntent(intent: unknown): boolean {
  return typeof intent === "string" && LAUNCHER_INTENTS.has(intent);
}

export function isGovernedIntent(intent: unknown): boolean {
...
export function validateCharter(workspace: any): string | null {
...
export function validateTriggerType(action: any): string | null {
...
export function validateTrigger(action: any): string | null {
  const triggerType = action?.trigger?.type;
  if (!ALLOWED_TRIGGERS.has(triggerType)) {
    return "Invalid trigger type";
  }
...
export function validateIntent(action: any): string | null {
...
export function validateRecordedIntent(action: any): string | null {
...
function isIsoDate(input: unknown): boolean {
...
export function parseArchieve(archieve: any):
  | {
      ok: true;
      fileStem: string;
      typeKey: string;
      explicitType: boolean;
      retention: { retention: string; route: string };
      normalized: {
        dept: string;
        type: string;
        date: string;
...
function scanStrings(input: unknown, out: string[]): void {
...
export function detectInjection(input: unknown): boolean {
  const strings: string[] = [];
  scanStrings(input, strings);
  return strings.some((value) => INJECTION_PATTERNS.some((pattern) => pattern.test(value)));
}

export function detectEmergencyJustification(evidence: any): boolean {
...
export function toConnector(target: string):
  | "sharepoint"
  | "powerautomate"
  | "aad"
  | "civicplus"
...
export const CONNECTOR_PERMISSIONS: Record<string, string[]> = {
  sharepoint: ["deploy"],
