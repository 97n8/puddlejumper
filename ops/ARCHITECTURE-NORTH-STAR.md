...
- **Primitive audit payloads: typed JSON plus JSON-path indexes.** New primitives write typed payloads into `audit_events.metadata` and add JSON-path indexes only for the fields they need to query; they do not add primitive-specific columns to `audit_events`. Rationale: keep the shared audit schema stable while letting each primitive own its payload shape. First instance: Relay V1.
- **Migrations: forward-only SQL with per-database history.** Schema changes live in `migrations/YYYYMMDD_snake_case_description.sql`. Each file begins with four required headers (`Database`, `Description`, `Author`, `Date`) and targets exactly one database, because SQLite transaction boundaries are per-database. Migration history lives in a `schema_migrations` table inside each target database, not centrally. Files are forward-only, checksum-enforced, and immutable once applied.
- **Migration validation set: earned, not assumed.** Only databases with at least one reviewed migration file belong in the runner's accepted `Database` header set. Self-initialized or inline-schema databases (currently: `dog`, `commons`, `oauth_state`, `refresh_tokens`, `rate_limit`, `idempotency`) stay out until their schemas are formally migrated onto reviewed files. Attempting to write a migration targeting an unrecognized database name must fail loud.
